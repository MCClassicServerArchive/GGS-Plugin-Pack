 ...
 */
package com.ep.ggs.groupmanager.API;
...
public class GroupManagerAPI {
	 ...
	 * @param isop
	 *            If the group is an operator group
	 * @return If successful
	 */
	public static boolean createGroup(String name, int permission, ChatColor color,
			boolean isop) {
	 ...
	 *            the group name
	 * @param isop
	 *            if the group should be operator
	 * @return if successful
	 */
	...
	public static String[] listGroups() {
		...
		for (int i = 0; i < names.length; i++) {
			names[i] = Group.getGroupList().toArray(
					new Group[Group.getGroupList().size()])[i].name;
		}
		return names;
	}
}
